...
def get_generator(filepath="/large_data/wikitext-103/wiki.valid.tokens", load_strategy="default", load_data=[False, ""],
    ...
    return wiki_loader.get_tf_dataset_generator(process_strategy, shuffle, pad, window_size, "<lm>", "<confidence>").batch(batch_size)
...
if __name__ == "__main__":
    ...
    batch_size = 8*4 # last number is the number of gpus, first is the batch size per gpu.
    parallel_layers = {}
    ...
    learning_rate = tf.keras.optimizers.schedules.ExponentialDecay(0.0001, decay_steps=1500,
    ...
    train_class = ParentTrainNL(transformer, optimizer, loss_object, loss_function_sequence_split, tokenizer,
                 ...
                 recent_to_keep=100, load_recent=False, best_to_keep=5, load_best=False)
    train_class.train_iteration(epoch_start=0, epoch_end=2, save_filepath_train="../../results/",

...
if __name__ == "__main__":
    ...
    dloader = MasterDataLoaderTF(filepaths=filepaths, seq_len=config.max_seq_len_dec, batch_size=config.batch_size, tokenizer=config.tokenizer) # all of the other parameters are equal to the default value, so not included here.
    generator_train = dloader.get_generator("RACE_middle_train", False).batch(config.batch_size)
    generator_val = dloader.get_generator("RACE_middle_val", False).batch(config.batch_size)
    ...
    train_class = NMTransformerEncDecTrain(transformer, optimizer, config.loss_object, loss_function_end_tok_only, config.tokenizer,
                                     checkpoint_path_recent="/home/kkno604/Documents/Final_model_pretraining/Checkpoints/RACE_middle_train/",
    ...
    train_class.train_iteration(epoch_start=0, epoch_end=20, iteration_counter=0,
                                save_filepath_train="/home/kkno604/Documents/Final_model_pretraining/Results/RACE_middle_train/",
                                save_filepath_val="/home/kkno604/Documents/Final_model_pretraining/Results/RACE_middle_train/",
                                data_dict=data_dict, num_aux_tokens=config.num_aux_tokens, save_end_epoch=True,
                                print_every_iterations=5, save_every_iterations=50)

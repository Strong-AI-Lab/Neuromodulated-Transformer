...
if __name__ == "__main__":
    ...
    filepaths = {"PIQA_test": {"question": "/large_data/PIQA/valid.jsonl",
                               "labels": "/large_data/PIQA/valid-labels.lst"}}

    dloader_test = MasterDataLoaderTF(filepaths=filepaths, seq_len=config.max_seq_len_dec, batch_size=config.batch_size,
                                      tokenizer=config.tokenizer)

    generator_test = dloader_test.get_generator("PIQA_test", False, override_lm=False).batch(config.batch_size)
    ...
    for i in range(1,21):
        ...
        train_class = FineTuningClass(transformer, optimizer, config.loss_object, loss_function, config.tokenizer,
                                      checkpoint_path_recent="/data/kkno604/Specifically_fine_tuned_general_model/PIQA/checkpoints/",
                                      ...
                                      load_specific_path="/data/kkno604/Specifically_fine_tuned_general_model/PIQA/checkpoints/ckpt-"+str(240+i),
                                      #load_specific_path="/home/kkno604/Documents/V4 results/Specific-fine-tuning/RACE/Checkpoints/saved_checkpoints/ckpt-209",
                                      #load_specific_path="",
                                      enc_tok="<enc>", dec_tok="<dec>",
                                      output_layer_name="mqa", fixed_output=True, stop_gradient=False,
        ...
        train_class.get_test_results(e=i-1,
                                     save_filepath="/data/kkno604/Specifically_fine_tuned_general_model/PIQA/results/val/",
                                     data=data_dict["test"], num_aux_tokens=config.num_aux_toks,
                                     max_generate_len=1, filename_prefix="PIQA-val-ckpt-"+str(i), metrics=["accuracy"],

...
class LAMBADADataLoader:
    ...
    def load_data(self, filepath):
        ...
        for i in range(len(self.data)):
            self.data[i] = re.sub("\n", "", self.data[i])
            self.data[i] = re.sub("`", "", self.data[i])
            self.data[i] = re.sub("'", "", self.data[i])
        #print(self.data)
...
if __name__ == "__main__":
    ...
    dloader = LAMBADADataLoader(strategy="doesn't even matter",
                             filepath="/large_data/LAMBADA/dataset/lambada_test_plain_text.txt",

...
class PTBDataLoader:
    ...
    def __init__(self, strategy: str, filepath: str, enc_tok: str, dec_tok: str, mlm_tok: str, lm_tok: str,
        ...
        assert self.strategy in ["line-by-line", "join-lines"], f"The strategy {self.strategy} is not supported!"
    ...
    def call__(self):
        ...
        if not isinstance(self.data, str):
            for row in self.data: #row is string...
                article_ids = self.tokenizer.encode_single_id_string_max_seq_len(row, max_seq_len=10000000)[0]
                # [1:-2] # 0 is ids, 1 is strings. # no <s> or </s>
            ...
            article_ids = self.tokenizer.encode_single_id_string_max_seq_len(self.data, max_seq_len=10000000)[0]
            # [1:-2] # 0 is ids, 1 is strings. # no <s> or </s>
            # article_ids = article_ids["input_ids"]
            start_index = 0
            end_index = self.seq_len
            ...
            while True:
                ...
                if end_index < article_len - 1:
                    ...
                    tar_inp = article_ids[
                              start_index:article_len - 1]  # -1 so there is an output token for the target.
                    tar_real = article_ids[start_index + 1:article_len]  # i.e. shifted to the right.
...
if __name__ == "__main__":
    ...
    dloader = PTBDataLoader(strategy="doesn't even matter",

...
class BoolQCSDataLoader:
    ...
    def __call__(self, mode: str, shuffle: bool):
        ...
        if mode == "test":
            ...
            '''
            Note: we remove answers that have an empty pertubated question and no answer.
            '''

            #print(self.data.keys())
            for i, dict_ in enumerate(self.data["data"]):
                #print(f"\n\ndict_: {dict_}\n\n")
                if i == 0: continue # This first dict_ is just explaining the structure... skip it.
                ...
                skip = False
                answer = None
                # print(f"{perturbed_dict}")
                if dict_["answer"] == "TRUE":
                    answer = "yes"
                elif dict_["answer"] == "FALSE":
                    answer = "no"
                else: skip = True

                if not skip:
                    inp_ = self.p1 + " " + dict_["paragraph"] + " " + self.question + \
                ...
                for perturbed_dict in dict_["perturbed_questions"]:

                    answer = None
                    #print(f"{perturbed_dict}")
                    if perturbed_dict["answer"] == "TRUE":
                        answer = "yes"
                    elif perturbed_dict["answer"] == "FALSE":
                        answer = "no"
                    else: continue
                        #raise Exception(f"Error, answer isn't TRUE or FALSE: {perturbed_dict['answer']}")

                    inp_ = self.p1 + " " + dict_["paragraph"] + " " + self.question + \
                           " " + perturbed_dict["perturbed_q"] + " " + self.sep_tok
...
if __name__ == "__main__":
    ...
    dloader = BoolQCSDataLoader(strategy=None,
                             #filepath="/large_data/BoolQ/train.jsonl",
                              filepath = "/large_data/contrast-sets/BoolQ/boolq_perturbed.json",

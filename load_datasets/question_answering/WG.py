...
class WGDataLoader:
    '''
    The implementation of a dataloader for the PIQA dataset to be utilzed by another higher level data loader.
    '''
    ...
    def __init__(self, strategy: str, filepath: str, enc_tok: str, dec_tok: str, mlm_tok: str, lm_tok: str,
        ...
        if tokenizer is not None:
            ...
            if len(self.psqa_tok_id) != 1 and (isinstance(self.psqa_tok_id, list)):
                raise Exception(f"The number of ids the end token is encoded into should be one, got {len(self.psqa_tok_id)}!")
            else:
    ...
    def load_data(self):
        ...
        self.data = None
        with open(self.filepath["question"], "r") as f: # note need .jsonl file.
            self.data = [json.loads(line) for line in f]

        self.labels = None
        with open(self.filepath["labels"], "r") as f:
            self.labels = f.readlines() # returns a list containing each item.
    ...
    def __call__(self, mode: str, shuffle: bool):
        ...
        if mode == "train_label" or mode == "val_label":
            for i, dict_ in enumerate(self.data):
                ...
                question_stem = dict_["sentence"]
                answer_options = [dict_["option1"], dict_["option2"]]
                #print(f"self.labels[i]\t{self.labels[i].strip()}")
                cor_answer = self._map_latin_to_int_helper(dict_["answer"])  #+1 b/c start at 0 and we start with a1 as 1.

                ques = self.question + " " + question_stem + " "
                ques += self.a1 + " " + answer_options[0] + " " + self.a2 + " " + answer_options[1] + " " + self.sep_tok

                data_id_string = self.tokenizer.encode_single_id_string_max_seq_len(ques, max_seq_len=10000000)  # [0] is ids [1] is string version...
                label_data_id_string = self.tokenizer.encode_single_id_string_max_seq_len(cor_answer, max_seq_len=1000000)
        ...
        if mode == "train_hide_label" or mode == "val_hide_label":
            for i, dict_ in enumerate(self.data):
                ...
                label_data_id_string_answer = self.tokenizer.encode_single_id_string_max_seq_len(cor_answer,max_seq_len=1000000)
            ...
            for i, dict_ in enumerate(self.data):
                ...
                answer_options = [dict_["option1"], dict_["option2"]]
                # print(f"self.labels[i]\t{self.labels[i].strip()}")
                label = self._map_latin_to_int_helper(dict_["answer"])  # +1 b/c start at 0 and we start with a1 as 1.

                ques = self.question + " " + question_stem + " "
                ques += self.a1 + " " + answer_options[0] + " " + self.a2 + " " + answer_options[1] + " " + self.sep_tok

                all_labels = self.a1 + " " + answer_options[0] + " " + self.a2 + " " + answer_options[1] + " "
                #self._map_latin_to_int_helper(a["label"]) + " " + a["text"] + " "
...
if __name__ == "__main__":
    ...
    dloader = WGDataLoader(strategy="train",
                             filepath={"question":"/large_data/winogrande/winogrande/data/dev.jsonl",

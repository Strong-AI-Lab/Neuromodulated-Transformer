...
class SIQADataLoader:
    '''
    The implementation of a dataloader for the SIQA dataset to be utilzed by another higher level data loader.
    '''
    ...
    def __call__(self, mode: str, shuffle: bool):
        ...
        if mode == "train_label" or mode == "val_label":
            for i, dict_ in enumerate(self.data):
                ...
                passage = dict_["context"]
                question_stem = dict_["question"]
                answer_options = [dict_["answerA"], dict_["answerB"], dict_["answerC"]]
                #print(f"self.labels[i]\t{self.labels[i].strip()}")
                cor_answer = self._map_latin_to_int_helper(str(int(self.labels[i])).strip())  #+1 b/c start at 0 and we start with a1 as 1.

                ques = self.p1 + " " + passage + " " + self.question + " " + question_stem + " "
                ques += self.a1 + " " + answer_options[0] + " " + self.a2 + " " + answer_options[1] + " " +\
                        self.a3 + " " + answer_options[2] + " " + self.sep_tok
            ...
            for i, dict_ in enumerate(self.data):
                ...
                answer_options = [dict_["answerA"], dict_["answerB"], dict_["answerC"]]
                # print(f"self.labels[i]\t{self.labels[i].strip()}")
                label = self._map_latin_to_int_helper(str(int(self.labels[i])).strip())  # +1 b/c start at 0 and we start with a1 as 1.
...
if __name__ == "__main__":
    ...
    dloader = SIQADataLoader(strategy="train",
                             filepath={"question":"/large_data/SIQA/socialiqa-train-dev/train.jsonl",
                                       "labels":"/large_data/SIQA/socialiqa-train-dev/train-labels.lst"},

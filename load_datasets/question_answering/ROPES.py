...
class ROPESDataLoader:
    '''
    The implementation of a dataloader for the ROPES dataset to be utilzed by another higher level data loader.
    '''
        ...
        if mode == "training" or mode == "val" or mode == "valid":
            ...
            for item in self.data:
                ...
                for paragraph in item["paragraphs"]: #{dict}
                    ...
                    context = paragraph["background"] + " " + paragraph["situation"]
                    quest_ = ""
                    answer = ""
            ...
            for item in self.data:
                ...
                for paragraph in item["paragraphs"]: #{dict}
                    ...
                    for qas in paragraph["qas"]: #[{questions:"",id:"",answers:[{text:'',answer_start}]},is_impossible:bool, plausible_answers:[same as answers list.]]
                        ...
                        if len(answers) < 9: answers += ["<pad>" for _ in range(9-len(answers))]
...
if __name__ == "__main__":
    ...
    dloader = ROPESDataLoader(strategy="train",
                             filepath="/large_data/ROPES/ropes-train-dev-v1.0/dev-v1.0.json",

...
class DROPDataLoader:
    '''
    The implementation of a dataloader for the DROP dataset to be utilzed by another higher level data loader.
    '''
    ...
    def __call__(self, mode: str, shuffle: bool):
        ...
        if mode == "training" or mode == "val" or mode == "valid":
            raise Exception(f"Training/Val path is not tested correctly.")
        elif mode == "test":

            for key in self.data.keys():

                passage = self.data[key]["passage"]

                for qas in self.data[key]["qa_pairs"]:
                    quest_ = qas["question"]

                    #go through validated answers
                    answers = []
                    #break_ = True
                    if "validated_answers" in qas.keys():
                        for dict_answers in qas["validated_answers"]:

                            ## check number
                            if dict_answers["number"] != "":
                                answers.append(dict_answers["number"])
                            elif dict_answers["date"]["day"] != "" or dict_answers["date"]["month"] != "" or \
                                    dict_answers["date"]["year"] != "" :
                                str_ = ""
                                str_ += dict_answers["date"]["day"]
                                if dict_answers["date"]["day"] != "": str_ += " "
                                str_ += dict_answers["date"]["month"]
                                if dict_answers["date"]["month"] != "": str_ += " "
                                str_ += dict_answers["date"]["year"]
                                answers.append(str_.strip())
                                #break_ = False
                            elif len(dict_answers["spans"]) != 0:
                                for span in dict_answers["spans"]:
                                    answers.append(span)
                            else: raise Exception(f"No answer provided, raising error!")
                    else: # this pathway is for the contrast set.
                        dict_answers = qas["answer"]
                        if dict_answers["number"] != "":
                            answers.append(dict_answers["number"])
                        elif dict_answers["date"]["day"] != "" or dict_answers["date"]["month"] != "" or \
                                dict_answers["date"]["year"] != "":
                            ...
                            raise Exception(f"No answer provided, raising error!")


                    if len(answers) < 20: answers += ["<pad>" for _ in range(20-len(answers))]
                    #if break_: continue
                    tmp = self.p1 + " " + passage + " " + self.question + " " + quest_ + " " + self.sep_tok
...
if __name__ == "__main__":
    ...
    dloader = DROPDataLoader(strategy="train",
                             filepath="/large_data/DROP/drop_dataset_with_test_questions/drop_dataset_dev.json",

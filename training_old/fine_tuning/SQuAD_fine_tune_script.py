...
from training.fine_tuning.SQuAD_fine_tune_train import *
...
if __name__ == "__main__":
    ...
    generator_train = dloader_train.get_generator("SQuAD_train_default", True).batch(config.batch_size)
    ...
    dloader_val = MasterDataLoaderTF(filepaths=filepaths, seq_len=config.max_seq_len_dec,
                                       batch_size=config.batch_size, tokenizer=config.tokenizer)
    generator_val = dloader_val.get_generator("SQuAD_val_default", False).batch(config.batch_size)

    data_dict["val"] = generator_val
    if strategy is not None:
        data_dict["val"] = strategy.experimental_distribute_dataset(data_dict["val"])

    train_class = SQuADFineTuning(transformer, optimizer, config.loss_object, loss_function, config.tokenizer,
                                     checkpoint_path_recent="/home/kkno604/Documents/V4 results /Specific-fine-tuning/SQuAD/Checkpoints/Final_v4_model/",
    ...
    train_class.train_batch(epoch_start=30, epoch_end=33, iteration_counter=0,
                            save_filepath_train="/home/kkno604/Documents/V4 results /Specific-fine-tuning/SQuAD/Results/Final_v4_model/",
                            save_filepath_val="/home/kkno604/Documents/V4 results /Specific-fine-tuning/SQuAD/Results/Final_v4_model/",

...
if __name__ == "__main__":
    ...
    generator_train = dloader_train.get_generator("CQA_train_hide_label", True, override_lm=False).batch(
        config.batch_size)
    ...
    generator_val = dloader_val.get_generator("CQA_val_hide_label", False, override_lm=False).batch(config.batch_size)
    ...
    train_class = FineTuningClass(transformer, optimizer, config.loss_object, loss_function, config.tokenizer,
                                  checkpoint_path_recent="/data/kkno604/Reading_strategy_experiments/aoint_only/CQA/Checkpoints/",
    ...
    raise Exception(f"Not implemented correctly yet!")

    cls_tok_id = config.tokenizer.encode_single("<cls>")[0]
    dec_tok_id = config.tokenizer.encode_single("<dec>")[0]
    lst_of_task_ids = []
    for aux_tok in ["<lm>", "<mqa>", "<gqa>"]:
        lst_of_task_ids.append(config.tokenizer.encode_single(aux_tok)[0])

    train_class.model.build_custom(cls_tok_id, dec_tok_id, lst_of_task_ids, minval=1, maxval=24)

    print(train_class.model.summary())
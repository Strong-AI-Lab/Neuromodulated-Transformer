...
class ExternalMemory(tf.keras.layers.Layer):
    ...
    def call(self, x, training):
        ...
        #if self.memory_capacity == 0: return tf.zeros((x.shape[0], x.shape[1], x.shape[2]))

        word_attn_memory = None  # Will get to the following shape: (batch_size, memory_size, seq_len, d_model)
        # attend over each word
        for i in range(
                self.memory_capacity):  # note that order doesn't really matter because bidirectional GRU/LSTM/RNN used mostly.
            # print(f"i: {i}")
            # index = tf.convert_to_tensor([i])
            mem = self.memory[i] if self.memory_capacity > 0 else tf.zeros((x.shape[0], x.shape[1], x.shape[2]))
            z = self._word_attn(x, mem)  # self.memory(i) returns the tensor at index/slot i.
            # print("REACHHH")
            # z.shape = (batch_size, seq_len)
            z = tf.repeat(tf.expand_dims(z, -1), mem.shape[-1], axis=-1)  # (batch_size, seq_len, d_model)
            z = z * mem  # appy the attention weights to the memory.
        ...
        word_attn_memory = word_attn_memory if word_attn_memory is not None else tf.zeros(
        ...
        # else means run through once even though it is zero to build the weights.
        out_tensor = None
    ...
    def _row_col_attn(self, x, training=True):
        ...
        word_attn_memory = None # Will get to the following shape: (batch_size, memory_size, seq_len, d_model)
        # attend over each word
        for i in range(self.memory_capacity): # note that order doesn't really matter because bidirectional GRU/LSTM/RNN used mostly.
            #print(f"i: {i}")
            #index = tf.convert_to_tensor([i])
            z = self._word_attn(x, tf.stop_gradient(self.memory[i])) #self.memory(i) returns the tensor at index/slot i.
            #print("REACHHH")
            #z.shape = (batch_size, seq_len)
            z = tf.repeat(tf.expand_dims(z, -1), self.memory[i].shape[-1], axis=-1) # (batch_size, seq_len, d_model)
